...
# 图幅大小
img_xsize = 3000  # 一种可能更快的方式是不创建图，而是导入图。不清楚创建大背景图（本质是创建numpy数组）是不是很快。
img_ysize = 1500
# 绘点大小
point_r = 25  # 所绘制点的半径大小（重要参数，值得调整，是大还是小？）
# 串口参数
...
Param2 = 30  # 它越小，可以检测到更多的假圆。它越大，能通过检测的圆就更加接近完美的圆形。
    ...
    while True:
        ...
        if angle is not None:
            ...
            if start_angle1 <= angle <= start_angle2 and not draw_flag:  # 当draw_flag为True时不再进行重置。即不重复重置。
                img_black = np.zeros((img_ysize, img_xsize), np.uint8)  # 重新生成黑色背景图
                draw_flag = True
            # 两区之间、1/4象限进行绘图（只有判断完起始区才能进行如下绘图）
            if draw_flag and not (90 <= angle <= 270):
                # ——————关键绘图函数——————
            ...
            if end_angle1 <= angle <= end_angle2 and draw_flag:  # 必须要先经历起始区，才能触发结束区。
                ...
                # 调试用
                img_black = cv2.resize(img_black, None, fx=0.25, fy=0.25, interpolation=cv2.INTER_AREA)  # 缩小显示，防止图像过大。
